...
- Prefer an installed xAI Docs MCP server if one exists in the current Claude Code session.
...
- xAI hosts an official docs MCP server at `https://docs.x.ai/api/mcp` (configured in `.claude/mcp.json`).
