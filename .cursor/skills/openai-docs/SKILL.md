---
name: openai-docs
description: Provides current, citation-backed guidance from official OpenAI documentation. Use when the user asks about OpenAI APIs, models, SDKs, Responses API, Chat Completions, Realtime, Agents SDK, Apps SDK, Codex, GPT-5.4 upgrades, prompt-upgrade guidance, or choosing the latest OpenAI model. Prefer an installed OpenAI docs MCP server when available; otherwise search and fetch only official OpenAI docs pages.
---

# OpenAI Docs

Adapted for Cursor from OpenAI's upstream `openai-docs` skill:
`https://github.com/openai/skills/tree/main/skills/.system/openai-docs`

This version is Cursor-native. It does not assume Codex-specific tool names or `codex mcp add`.

## Quick start

- Prefer an installed OpenAI docs MCP server if one exists in the current Cursor environment.
...
- This skill is a Cursor adaptation of OpenAI's upstream `openai-docs` skill.
- The original Codex-specific MCP install instructions were intentionally removed because they are not portable to Cursor as-is.
