...
**AmigaGPT** supports multiple speech providers, configurable from **Speech Provider Settings**. For cloud-based voices, it supports **OpenAI TTS** (high quality 16-bit voices), **SpaceXAI Grok TTS**, **ElevenLabs**, and **OpenVox** (requires AHI). For offline synthesis, AmigaOS 3 can use the Amiga's built-in `narrator.device` (**v34** and **v37**) with `translator.library`, and AmigaOS 4.1 supports `flite.device`. The **Speech** tab generates reusable WAV files from the active speech profile without playing them automatically; **Play**, **Pause**, **Stop** and **Rewind** read the saved file locally, and the waveform timeline can be clicked to jump. Speech also runs in the background from Chat replies or ARexx. While a request is running, **Send**, **Create** and **Generate** change to **Stop** and abort that request.
