...
**CJK (Chinesisch/Japanisch/Koreanisch):** CJK-Zeichen werden in Scintilla.mcc als □ (Tofu) angezeigt, da DejaVu keine CJK-Glyphen enthält. **Scintilla.mcc auf MorphOS ignoriert `SCI_STYLESETFONT` vollständig** — weder Family-Name noch Dateipfad bewirken einen Font-Wechsel; der Font wird intern von der MCC festgelegt (vermutlich über MUI-Prefs). Style-Segmentierung mit separatem CJK-Font (z. B. WenQuanYi Micro Hei) ist daher **nicht umsetzbar**. Der Code für `utf8_augment_cjk_styles()` und `CHAT_OUTPUT_STYLE_CJK` wurde nach Tests entfernt. Live-Scintilla-Refresh während des Streams mit CJK kann MorphOS/TTEngine **einfrieren** — der Fork überspringt deshalb Live-Redraw und Netz-TTS bei erkanntem CJK (`utf8_contains_cjk()`); Anzeige einmal nach Stream-Ende. **Export** enthält korrektes UTF-8 einschließlich CJK. Ein zukünftiges eigenes TextView.mcc könnte CJK-Fonts per TTEngine direkt ansprechen.
