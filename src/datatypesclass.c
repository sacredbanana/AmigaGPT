...
#ifdef __AMIGAOS3__
__attribute__((__saveds__)) ULONG
DataTypes_Dispatcher(struct IClass *cl __asm("a0"), Object *obj __asm("a2"),
                     Msg msg __asm("a1"));
#endif
...
static union printerIO *CreatePrtReq(void) {
    ...
    if ((mp = CreateMsgPort())) {
        if ((pio = (union printerIO *)CreateIORequest(mp,
...
STATIC ULONG DataTypes_New(struct IClass *cl, Object *obj, struct opSet *msg) {
    ...
    if (!(obj = (Object *)DoSuperMethod(cl, obj, OM_NEW, MUIA_FillArea, TRUE,
...
CreateMCC(CONST_STRPTR supername, struct MUI_CustomClass *supermcc,
          int instDataSize,
          ULONG (*dispatcher)(struct IClass *, Object *, Msg)) {
    struct MUI_CustomClass *cl = MUI_CreateCustomClass(
        NULL, supername, supermcc, instDataSize, (void *)dispatcher);
    return cl;
}
#elif defined(__MORPHOS__)
struct MUI_CustomClass *CreateMCC(CONST_STRPTR supername,
                                  struct MUI_CustomClass *supermcc,
                                  int instDataSize, APTR dispatcher) {
    extern ULONG muiDispatcherEntry(void);

    struct MUI_CustomClass *cl;

    if ((cl = MUI_CreateCustomClass(NULL, supername, supermcc, instDataSize,
                                    &muiDispatcherEntry))) {
...
struct MUI_CustomClass *CreateMCC(CONST_STRPTR supername,
                                  struct MUI_CustomClass *supermcc,
                                  int instDataSize, APTR dispatcher) {
    return MUI_CreateCustomClass(NULL, supername, supermcc, instDataSize,
                                 ENTRY(DataTypes_Dispatcher));
}
#endif
...
STATIC ULONG DataTypes_Set(struct IClass *cl, Object *obj, struct opSet *msg) {
    ...
    while ((tag = NextTagItem(&tstate))) {
        ...
        case MUIA_DataTypes_HorizScrollbar:
            ...
            DoMethod(data->horiz_scrollbar, MUIM_Notify, MUIA_Prop_First,
                     MUIV_EveryTime, (ULONG)obj, 1, MUIM_Datatypes_HorizUpdate);
            break;

        case MUIA_DataTypes_VertScrollbar:
            data->vert_scrollbar = (Object *)tidata;
            DoMethod(data->vert_scrollbar, MUIM_Notify, MUIA_Prop_First,
                     MUIV_EveryTime, (ULONG)obj, 1, MUIM_Datatypes_VertUpdate);
            break;
        }
    }
    ...
    if (newfilename || newbuffer) {
        ...
        MUI_Redraw(obj,
                   MADF_DRAWOBJECT); // Trigger a redraw to show "Loading..."

        data->dt_obj =
            NewDTObject(newfilename, PDTA_DestMode, PMODE_V43, TAG_DONE);

        if (data->dt_obj) {
            /* If is between MUIM_Show and MUIM_Hide add the datatype to the
             * window */
            if (data->show) {
                SetDTAttrs(data->dt_obj, NULL, NULL, GA_Left, _mleft(obj),
                           GA_Top, _mtop(obj), GA_Width, _mwidth(obj),
                           GA_Height, _mheight(obj), ICA_TARGET, ICTARGET_IDCMP,
                           TAG_DONE);
                DoMethod(data->dt_obj, PDTM_SCALE, _mwidth(obj), _mheight(obj),
                         0);
...
STATIC ULONG DataTypes_AskMinMax(struct IClass *cl, Object *obj,
...
STATIC ULONG DataTypes_Setup(struct IClass *cl, Object *obj,
...
STATIC ULONG DataTypes_Show(struct IClass *cl, Object *obj, Msg msg) {
    ...
    if (data->original_filename) {
        ...
        data->dt_obj = NewDTObject(data->original_filename, PDTA_DestMode,
                                   PMODE_V43, TAG_DONE);

        SetDTAttrs(data->dt_obj, NULL, NULL, GA_Left, _mleft(obj), GA_Top,
                   _mtop(obj), GA_Width, _mwidth(obj), GA_Height, _mheight(obj),
                   ICA_TARGET, ICTARGET_IDCMP, PDTA_DestMode, PMODE_V43,
                   TAG_DONE);
...
STATIC ULONG DataTypes_Draw(struct IClass *cl, Object *obj,
    ...
    if (msg->flags & MADF_DRAWOBJECT) {
        if (data->loading) {
            DoMethod(obj, MUIM_Text, _mleft(obj),
                     _mtop(obj) + (_mheight(obj) >> 1), _mwidth(obj),
                     _mheight(obj), "Loading image...", -1, "\033c", 'b');
        } else if (data->dt_obj) {
...
STATIC ULONG DataTypes_HandleEvent(struct IClass *cl, Object *obj,
    ...
    if (msg->imsg && msg->imsg->Class == IDCMP_IDCMPUPDATE) {
        ...
        while ((tag = NextTagItem(&tstate))) {
            ...
            switch (tag->ti_Tag) {
            case DTA_Busy:
                /*							if (tidata) set(_app(obj),
                   MUIA_Application_Sleep, TRUE); else set(_app(obj),
                   MUIA_Application_Sleep, FALSE);*/
                break;
...
STATIC ULONG DataTypes_Print(struct IClass *cl, Object *obj, Msg msg) {
    ...
    if (data->dt_obj && !data->pio) {
        if ((data->pio = CreatePrtReq())) {
            if (OpenDevice("printer.device", 0 /* should be user selectable */,
                           (struct IORequest *)data->pio, 0) == 0) {
                ...
                if (PrintDTObject(data->dt_obj,
                                  data->show ? _window(obj) : FALSE, NULL,
                                  DTM_PRINT, NULL, data->pio, tags)) {
                    /*
                                        data->ihnode.ihn_Object = obj;
                                        data->ihnode.ihn_Signals = 1UL <<
                       (data->pio->ios.io_Message.mn_ReplyPort);
                                        data->ihnode.ihn_Flags = 0;
                                        data->ihnode.ihn_Method =
                       MUIM_DataTypes_PrintCompleted;
...
STATIC ULONG DataTypes_VertUpdate(struct IClass *cl, Object *obj) {
    ...
    if (!data->dt_obj)
        return 0;
    SetGadgetAttrs((struct Gadget *)data->dt_obj, _window(obj), NULL,
                   DTA_TopVert, xget(data->vert_scrollbar, MUIA_Prop_First));
    return 0;
}
...
STATIC ULONG DataTypes_HorizUpdate(struct IClass *cl, Object *obj) {
    ...
    if (!data->dt_obj)
        return 0;
    SetGadgetAttrs((struct Gadget *)data->dt_obj, _window(obj), NULL,
                   DTA_TopHoriz, xget(data->horiz_scrollbar, MUIA_Prop_First));
    return 0;
}
...
DISPATCHER(DataTypes_Dispatcher) {
    switch (msg->MethodID) {
    case OM_NEW:
        return DataTypes_New(cl, obj, (struct opSet *)msg);
    case OM_DISPOSE:
        DataTypes_Dispose(cl, obj, msg);
        return 0;
    case OM_SET:
        return DataTypes_Set(cl, obj, (struct opSet *)msg);
    case OM_GET:
        return DataTypes_Get(cl, obj, (struct opGet *)msg);
    case MUIM_AskMinMax:
        return DataTypes_AskMinMax(cl, obj, (struct MUIP_AskMinMax *)msg);
    case MUIM_Setup:
        return DataTypes_Setup(cl, obj, (struct MUIP_Setup *)msg);
    case MUIM_Cleanup:
        return DataTypes_Cleanup(cl, obj, msg);
    case MUIM_Show:
        return DataTypes_Show(cl, obj, msg);
    case MUIM_Hide:
        DataTypes_Hide(cl, obj, msg);
        return 0;
    case MUIM_Draw:
        return DataTypes_Draw(cl, obj, (struct MUIP_Draw *)msg);
    case MUIM_HandleEvent:
        return DataTypes_HandleEvent(cl, obj, (struct MUIP_HandleEvent *)msg);
    case MUIM_DataTypes_Print:
        return DataTypes_Print(cl, obj, msg);
    case MUIM_DataTypes_PrintCompleted:
        return DataTypes_PrintCompleted(cl, obj);
    case MUIM_Datatypes_VertUpdate:
        return DataTypes_VertUpdate(cl, obj);
    case MUIM_Datatypes_HorizUpdate:
        return DataTypes_HorizUpdate(cl, obj);
...
int create_datatypes_class(void) {
    if ((CL_DataTypes = MUI_CreateCustomClass(NULL, MUIC_Area, NULL,
                                              sizeof(struct DataTypes_Data),
                                              ENTRY(DataTypes_Dispatcher)))) {
